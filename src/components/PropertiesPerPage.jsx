...
import PropertyModal from './PropertyModal.jsx';
    ...
    const PropertiesPerPage = ({ properties,  showROI = false, propertiesPerPage, category = 'renta' }) => { 
    ...
    .filter((prop) => {
      ...
      const matchesPrice = Number(prop.precio) >= minPriceValue && Number(prop.precio) <= maxPriceValue;
      const matchesSearchTerm = 
                        prop.ubicacion?.toLowerCase().includes(searchTermLower) ||
                        prop.tipo?.toLowerCase().includes(searchTermLower);
      const matchesPropertyType = propertyType === '' || prop.tipo?.toLowerCase().trim() === propertyType.toLowerCase().trim();
